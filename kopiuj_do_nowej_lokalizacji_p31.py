...
print(
    "\nUWAGA!\nW lokalizacji podanej jako pierwsza może \
        utworzyć się plik z błędami."
)
print(
    "Program kopiuje pliki z rozszerzeniami:\n\
    .TXT, .WKT, .XML, .KCD, .DXF, .DWG, .DGN, .TFW, .DAT\n\n"
)
            ...
            for file in natsorted(files):
                if file.upper().endswith(
                    ...
                    try:
                        ...
                        with io.open(
                            os.path.join(stad, "nie_skopiowano.txt"),
                            "a",
                            encoding="utf-8",
                        ) as bledy:
                            bledy.write(plik + "\n")
