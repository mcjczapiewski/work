...
import cv2, os, datetime, numpy, fnmatch
...
for subdir, dirs, files in os.walk(sprawdzanie):
    ...
    for file in sorted(files):
        if (file.endswith('.jpg' or '.jpeg' or '.JPG' or '.JPEG')) and (plik_z_kolei in [1, countpliki//3, int(countpliki//1.5), countpliki-1]):
            pierwotne = file
                    ...
                    for file in sorted(files):
                        ...
                        if file.endswith('.jpg' or '.jpeg' or '.JPG' or '.JPEG'):
                            ...
                            drugie = os.path.join(subdir, porownane)
                                ...
                                #aby mogly byc te same obrazy, w naszym przypadku beda tez te same nazwy plikow, a wiec:
                            if pierwotne == porownane:

                                    #sprawdza czy plik oryginalny nie byl juz sprawdzany z drugim w odwrotnej kolejnosci (a,b = b,a), jak byly to bierze kolejny plik
                                ...
                                if czy_bylo in lines_seen:
                                    ...
                                    if pierwsze == drugie:
                                        ...
                                        try:
                                            ...
                                            #jak nie otworzy zdjecia to przechodzi do kolejnego po zapisaniu bledu do pliku tekstowego
                                        except:
                                            with open (bledny, 'a') as bl:
                                        ...
                                        try:
                                                #jesli ksztalty sa identyczne to sprawdza roznice w kanalach RGB
                                            if original.shape == image_to_compare.shape:
                                                difference = cv2.subtract(original, image_to_compare)
                                                b, g, r = cv2.split(difference)
        
                                                    #jesli roznice w kanalach wynosza 0 tzn. ze zdjecia sa identyczne
                                                if cv2.countNonZero(b) == 0 and cv2.countNonZero(g) == 0 and cv2.countNonZero(r) == 0:
                                                    ...
                                                    przerwa = 1
                                                        ...
                                                        #jesli znalazl w tym folderze odpowiadajaca pare, to konczy petle, bo jest duze prawdopodobiensto ze reszta tez bedzie tym samym
                                                        #wiec nie ma sensu sprawdzac wszystkiego
                                                    break
        
                                            #jak nie da sie ich otworzyc i porownac nawet ksztaltow to wypisze informacje do pliku
                                        except:
                                            with open (bledny, 'a') as bl:
