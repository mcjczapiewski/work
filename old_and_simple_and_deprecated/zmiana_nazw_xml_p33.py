...
count = 1
print(
    """\nTYLKO DO ZMIANY NAZW PLIKOW XML ZAWIERAJĄCYCH W POLU
    "nazwaPliku" ROZSZERZENIE .tif\n\n"""
)
sciezka = input("Podaj ścieżkę do folderu z plikami XML: ")
czy = input(
    'Aktualne rozszerzenie plików to txt czy xml? Wpisz "txt" lub "xml": '
)
if '"' in czy:
    czy = regex.sub('"(.+)"', "\\1", czy)
rozszerzenie = "." + czy
        ...
        if (file.lower()).endswith(rozszerzenie):
            ...
            with open(xml, "r") as plik:
                for line in plik:
                    if regex.match(r"^.+?nazwaPliku.*", line):
                        nazwa = regex.sub(
                            r"^.+?nazwaPliku>(.+)\.tif.+$", "\\1", line
                        )
                        nowy_xml = os.path.join(
                            subdir, nazwa.split("\n")[0] + ".xml"
                        )
            ...
            if not os.path.exists(nowy_xml):
                try:
                    ...
                    raise
                    with open(
                        os.path.join(sciezka, "BLEDY_XML.txt"), "a"
                    ) as bledy:
                ...
                with open(
                    ...
                    bledy.write(
                        ...
                        + "\t"
                        + nowy_xml
                        + "\tPlik o tej nazwie juz istnieje.\n"
                    )
