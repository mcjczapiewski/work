...
sciezki = r'D:\_MACIEK_\python_proby\p33\numer_p_do_xml\sciezki.txt'
count = 1
    ...
    for line in idz:
        ...
        for subdir, dirs, files in os.walk(sciezka):
            ...
            if not any(fname.upper().endswith('.XML') for fname in os.listdir(subdir)):
                with io.open(os.path.join(os.path.dirname(sciezki), 'bez_xml.txt'), 'a', encoding = 'utf-8') as bx:
                    bx.write(subdir+'\n')
            for file in natsorted(files):
                if file.upper().endswith('.XML') and file.upper().startswith('P.'):
                    ...
                    try:
                        with io.open(xml, 'r', encoding = 'utf-8') as r_xml:
                            for line in r_xml:
                                if regex.match('^.+pierwszyCzlon><', line):
                                    zapis = regex.sub('(^.+<pierwszyCzlon>)(.+$)', '\g<1>'+zasob+'\g<2>', line)
                                elif regex.match('^.+drugiCzlon><', line):
                                    zapis = regex.sub('(^.+<drugiCzlon>)(.+$)', '\g<1>'+powiat+'\g<2>', line)
                                elif regex.match('^.+trzeciCzlon><', line):
                                    zapis = regex.sub('(^.+<trzeciCzlon>)(.+$)', '\g<1>'+rok+'\g<2>', line)
                                elif regex.match('^.+czwartyCzlon><', line):
                                    zapis = regex.sub('(^.+<czwartyCzlon>)(.+$)', '\g<1>'+numer+'\g<2>', line)
                                else:
                                    zapis = line
                                tresc.append(zapis)
                        ...
                        with io.open(r'D:\_MACIEK_\python_proby\p33\numer_p_do_xml\bledy.txt', 'a', encoding = 'utf-8') as bl:
                            bl.write(xml+'\n')
                    except:
                        with io.open(r'D:\_MACIEK_\python_proby\p33\numer_p_do_xml\bledy.txt', 'a', encoding = 'utf-8') as bl:
                            bl.write(xml+'\tinny\n')
