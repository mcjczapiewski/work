# -*- coding: utf-8 -*-
    ...
    #import bibliotek
import os, datetime, ctypes, string
...
for subdir, dirs, files in os.walk(liczenie):
    dirs.sort()
    if not any(fname.upper().endswith(('.JPG', '.JPEG')) for fname in os.listdir(subdir)) or 'DOKUMENTACJA' in subdir or 'ZAŁOŻE' in subdir or 'MODERNIZACJA' in subdir or 'ponad' in subdir:
        continue
