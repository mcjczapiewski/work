...
def sciezki():
    ...
    sciezka = input()
    bledny = sciezka + '\\' + os.path.basename(os.path.normpath(sciezka)) + '_'\
        + czasstart.strftime('%Y-%m-%d') + '.txt'
    resztawkt = sciezka + '\\' + os.path.basename(os.path.normpath(sciezka))\
        + '_nieskopiowane_' + czasstart.strftime('%Y-%m-%d') + '.txt'
    ...
    for subdir, dirs, _ in os.walk(tusapdfy):
        ...
        for subdir, dirs, files in os.walk(tusawkt):
            ...
            for file in natsorted(files):
                if file.endswith('.wkt') and re.match('^' + nrope + r'(_|-| |\.wkt)', file):
...
def operaty():
    ...
    for subdir, dirs, _ in os.walk(tusapdfy):
        ...
        if not os.path.exists(plikwkt):
            with open(bledny, 'a') as bl:
                bl.write('WKT dla operatu nie istnieje więc nie skopiowano go na m-wyn, m-wyw itd.:\t')
                bl.write(folderpdf + '\n')
        ...
        # jeśli tak, to kopiuje go dla map i szkiców
            ...
            for _, _, files in os.walk(folderpdf):
                for file in natsorted(files):
                    if (re.match('(.*M-WYW.*)|(.*M-WYN.*)|(.*SZK-POL.*)', file)
                            and file.endswith(('.PDF', '.pdf'))):
                        newwkt = os.path.join(folderpdf, os.path.splitext(file)[0] + '.wkt')

                        # upewnia się, że wkt dla danej mapy/szkicu jeszcze nie istnieje, jesli
                        # nie to tworzy z głównego pliku
            ...
            # jeżeli nie ma w danym folderze wkt dla mapy wywiadu, wynikowej bądź żadnego szkicu
            # (bo nie ma dla nich pdf)
            # to tworzy wkt dla mapy uzupełniającej (jeśli istnieje)
            if (not any(re.match('(.*M-WYW.*wkt)|(.*M-WYN.*wkt)|(.*SZK-POL.*wkt)', fname)
