    ...
    for file in natsorted(files):
        if file.upper().endswith('.PDF') and file.upper().startswith('P.'):
            ...
            if not numer == kolejny:
                ...
                with io.open(os.path.join(sciezka, 'bledny_nr_dokumentu.txt'), 'a',
                             encoding='utf-8') as dokumentu:
            ...
            if file.upper().endswith('.PDF') and file.upper().startswith('P.'):
                ...
                if typ not in duble:
                    ...
                    if not str(pierwotna) == str(do_tego).zfill(3):
                        ...
                        with io.open(os.path.join(sciezka, 'bledny_numer_strony.txt'), 'a',
                                     encoding='utf-8') as strony:
