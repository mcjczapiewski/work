...
for subdir, dirs, files in os.walk(sprawdzanie):
    ...
    countpliki = len(fnmatch.filter(next(os.walk(folder_glowny))[2],
    ...
    for file in sorted(files):
        if ((file.endswith('.jpg' or '.jpeg' or '.JPG' or '.JPEG'))
                and (plik_z_kolei in [1, countpliki // 3, int(countpliki // 1.5), countpliki - 1])):
            pierwotne = file
            ...
            # jak nie otworzy to przechodzi do kolejnego zdjecia w folderze (continue)
            except:
                ...
                if folder_glowny in nie_powtarzaj and subdir in nie_powtarzaj:
                    ...
                    for file in sorted(files):
                        ...
                        if minelo >= 1:
                            ...
                            print('Na tym folderze upłynęło: ' + "%.2f" % czastrwania
                                  + 'min\nAktualnie sprawdzany plik (' + str(plik_z_kolei)
                                  + ' z ' + str(countpliki) + ' w tym folderze) porównano z '
                                  + str(countfiles) + ' zdjęciami.\n')
                            uplynelo = datetime.datetime.now()
                        ...
                        if file.endswith('.jpg' or '.jpeg' or '.JPG' or '.JPEG'):
                            ...
                            # aby mogly byc te same obrazy, w naszym przypadku beda tez te same nazwy
                            # plikow, a wiec:
                            if pierwotne == porownane:

                                # sprawdza czy plik oryginalny nie byl juz sprawdzany z drugim w
                                # odwrotnej kolejnosci (a,b = b,a), jak byly to bierze kolejny plik
                                czy_bylo = str(drugie + '\t' + pierwsze)
                                if czy_bylo in lines_seen:
                                    continue

                                # jesli jeszcze nie byly to dodaje ta kombinacje do pamieci, zeby
                                # przy kolejnych plikach znow sprawdzic
                                else:
                                    line = str(pierwsze + '\t' + drugie)
                                    lines_seen.add(line)

                                    # jesli sciezka pierwszego pliku zgadza sie ze sciezka drugiego
                                    # to ich nie powrownuje (ten sam plik)
                                    if pierwsze == drugie:
                                        continue

                                    # jesli jeszcze nie byly ze soba porownane, to je porownuje
                                    else:
                                        try:
                                            image_to_compare = cv2.imread(drugie)

                                        # jak nie otworzy zdjecia to przechodzi do kolejnego po
                                        # zapisaniu bledu do pliku tekstowego
                                        except:
                                            with open(bledny, 'a') as bl:
                                                bl.write(drugie + '\tnie otworzyło się\tDRUGIE\n')
                                            continue

                                    # podejmuje probe porownania
                                        try:
                                            # jesli ksztalty sa identyczne to sprawdza roznice
                                            # w kanalach RGB
                                            if original.shape == image_to_compare.shape:
                                                difference = cv2.subtract(original, image_to_compare)
                                                b, g, r = cv2.split(difference)

                                                # jesli roznice w kanalach wynosza 0 tzn. ze zdjecia
                                                # sa identyczne
                                                if (cv2.countNonZero(b) == 0 and cv2.countNonZero(g) == 0
                                                        and cv2.countNonZero(r) == 0):
                                                    with open(plikwynik, 'a') as wynik:
                                                        wynik.write('IDENTYCZNE\t' + pierwsze + '\t')
                                                        wynik.write(drugie + '\n')
                                                    nie_powtarzaj.add(folder_glowny)
                                                    nie_powtarzaj.add(subdir)
                                                    przerwa = 1

                                                    # jesli znalazl w tym folderze odpowiadajaca pare,
                                                    # to konczy petle, bo jest duze prawdopodobiensto
                                                    # ze reszta tez bedzie tym samym
                                                    # wiec nie ma sensu sprawdzac wszystkiego
                                                    break

                                        # jak nie da sie ich otworzyc i porownac nawet ksztaltow to
                                        # wypisze informacje do pliku
                                        except:
                                            with open(bledny, 'a') as bl:
                                                bl.write(pierwsze + '\t' + drugie)
                                                bl.write('\tnie zostały porównane\n')
                                            continue
