    ...
    with open(zbiorczy_wkt, "r") as zbiorczy:
        for line in zbiorczy:
            ...
            if "EMPTY" in line:
                ...
                continue
            nrnr = (
                (regex.split(r"\_\_\_", line)[2]).split("\n")[0]
                + "__"
                + regex.split(r"\_\_\_", line)[1]
            )
            ...
            if os.path.basename(subdir) == czy_pasi:
                if not os.path.exists(os.path.join(subdir, file)):
                    try:
                        ...
                        with open(
                            os.path.join(
                                os.path.dirname(zbiorczy_wkt), "BLEDY.txt"
                            ),
                            "a",
                        ) as bledy:
                            bledy.write(
                                os.path.join(tusawkt, file)
                ...
                if os.path.exists(wkt_operatu):
                    ...
                    if os.path.exists(new_wkt):
                        ...
                        try:
                            ...
                            with open(
                                os.path.join(
                                    os.path.dirname(operaty), "BLEDY.txt"
                                ),
                                "a",
                            ) as bledy:
                                bledy.write(
                                    new_wkt + "\tNie udało się utworzyć.\n"
                                )
                else:
                    with open(
                        os.path.join(os.path.dirname(operaty), "BLEDY.txt"),
                        "a",
                    ) as bledy:
