    ...
    for file in natsorted(files):
        if file.endswith(('.pdf', '.PDF')):
            ...
            for page in images_from_path:
                ...
                nazwa = os.path.join(subdir, nazwa_pdf + '__wpg_' + strona + '.jpg')
                try:
                    page.save(nazwa, 'JPEG', dpi=[300, 300])
                except MemoryError:
                    with open(bledy_druku, 'a') as bd:
                        bd.write('PDF do rozbicia PDFillem, memoryerror!:\t' + filename)
                        bd.write('\nSprawdzić pliki PDFa po nim, prawdopodobnie także trzeba poprawić.\n')
                    print('MEMORYERROR\tPlik musi zostać rozbity pdfillem\n' + filename
                          + '\nSprawdzić pliki PDFa po nim, prawdopodobnie także trzeba \
                          poprawić.\nPrzechodzę do kolejnego pliku.')
                    break
                except:
                    with open(bledy_druku, 'a') as bd:
                        bd.write('PDF do rozbicia PDFillem, nieokreślony bład!:\t' + filename)
                        bd.write('\nSprawdzić pliki PDFa po nim, prawdopodobnie także trzeba poprawić.\n')
                    print('Nieznany błąd.\tPlik musi zostać rozbity pdfillem\n' + filename
                          + '\nSprawdzić pliki PDFa po nim, prawdopodobnie także trzeba \
                          poprawić.\nPrzechodzę do kolejnego pliku.')
                    break
                nrstr += 1
            ...
            for entry in os.scandir(subdir):
                if entry.name.endswith('.ppm'):
                    try:
                        ...
                        with open(bledy_druku, 'a') as bd:
                            bd.write('PermissionError, możliwe że pliki ppm do usunięcia')
                            bd.write(' ręcznie w folderze!:\t' + subdir + '\n')
                        break
                    except:
                        with open(bledy_druku, 'a') as bd:
                            bd.write('Możliwe, że pliki ppm do usunięcia ręcznie w folderze!:\t')
                            bd.write(subdir + '\n')
                        break

            if Path(os.path.join(subdir, nazwa_pdf + '__wpg_001.jpg')).exists():
                continue
            else:
