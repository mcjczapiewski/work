...
##        with open(r'D:\_MACIEK_\cyfryzacja_wloclawski\G_brzesc\kodowania_plikow.txt', 'a') as kodowanie:
...
for subdir, dirs, _ in os.walk(r'D:\_MACIEK_\cyfryzacja_wloclawski\G_brzesc\utf-8'):
    dirs.sort(key=nkey)
    if os.path.exists(os.path.join(subdir, 'opis.txt')):
        stad = os.path.join(subdir, 'opis.txt')
        with io.open(stad, 'r', encoding='utf-8') as pobierz:
            tresc = pobierz.read()
        with io.open(stad+'.new', 'w', encoding='ascii', errors='ignore') as zapisz:
            zapisz.write(tresc)
