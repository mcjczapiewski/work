    ...
    with open(zbiorczy_wkt, 'r') as zbiorczy:
        for line in zbiorczy:
            ...
            if 'EMPTY' in line:
                ...
                nrope += 1
                continue
            nrnr = (regex.split(r'\_\_\_', line)[2]).split('\n')[0] + '__' + regex.split(r'\_\_\_', line)[1]
            line = regex.sub(r'\"(.+?)\".*$', r'\g<1>', line)
    ...
    with open(zbiorczy_wkt, 'r') as zbiorczy:
        for line in zbiorczy:
            ...
            if 'EMPTY' in line:
                ...
                nrope += 1
                continue
            nrnr = regex.search(r'^.+\)\)\).*?,\K.+?(?=,)', line)[0]
            if '\"' in nrnr:
                nrnr = (regex.split(r'\_\_\_', line)[2]).split('\n')[0] + '__'\
                    + regex.search(r'(")(.*?)(")', nrnr)[2]
            else:
                nrnr = (regex.split(r'\_\_\_', line)[2]).split('\n')[0] + '__' + nrnr
            nrope += 1
            line = regex.sub(r'\"(.+?)\".*$', r'\g<1>', line)
...
def teraz_kopiuj():
    ...
    for _, _, files in os.walk(tusawkt):
        for file in files:
            if '-' in file:
                czy_pasi = regex.search(r'__.+_', file)[0]
                czy_pasi = regex.sub(r'__|_$', '', czy_pasi)
                kopiuj()
            else:
                czy_pasi = regex.search(r'__.+\.wkt', file)[0]
                czy_pasi = regex.sub(r'__|\.wkt', '', czy_pasi)
                kopiuj()
...
def rozkopiowanie():
    for subdir, dirs, files in os.walk(operaty):
        for file in files:
            if regex.match(r'.+((-M-)|(-SZK-)).+\.PDF', file):
...
co_robimy = input('Co chcesz robić?:\n1 - utworzenie wkt dla operatów z csv\n2 - \
                  utworzenie dla wkt dla plików z csv\n3 - kopiowanie wkt do folderów operatów\n4 - \
                  rozkopiowanie istniejących wkt dla operatów do szkiców i map, które ich \
                  jeszcze nie mają\nPodaj cyfrę: ')
if co_robimy == '1':
    tusawkt = input('Podaj ścieżkę do zapisania plików wkt: ')
    zbiorczy_wkt = os.path.join(input('Podaj ścieżkę folderu, w którym znajduje się plik \
                                      dla_operatow.csv: '), 'dla_operatow.csv')
    operaty()
    czy_kopiuj()
elif co_robimy == '2':
    tusawkt = input('Podaj ścieżkę do zapisania plików wkt: ')
    zbiorczy_wkt = os.path.join(input('Podaj ścieżkę folderu, w którym znajduje się plik \
                                      dla_plikow.csv: '), 'dla_plikow.csv')
    pliki()
    czy_kopiuj()
elif co_robimy == '3':
