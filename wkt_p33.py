    ...
    for line in stad:
        ...
        with open(r"D:\_MACIEK_\python_proby\p33\lista_pdf.txt", "r") as tutaj:
            for line in tutaj:
                ...
                if nazwa1 == nazwa:
                    with open(
                        r"D:\_MACIEK_\python_proby\p33\co_do_czego.txt", "a"
                    ) as cdc:
...
with open(
    ...
    for line in stad:
        ...
        with open(
            r"D:\_MACIEK_\python_proby\p33\lista_plikow_p33.txt", "r"
        ) as tutaj:
            for line in tutaj:
                pelne1 = line.split("\n")[0]
...
with open(r"D:\_MACIEK_\python_proby\p33\co_do_czego.txt", "r") as cdc:
    for line in cdc:
        ...
        if linia != poprzednia:
            ...
            if czy1 == 1:
                with open(
                    r"D:\_MACIEK_\python_proby\p33\kilka_dopasowan.txt", "a"
                ) as kilka:
...
with io.open(
    r"D:\_MACIEK_\python_proby\p33\n_co_do_czego.txt", "r", encoding="utf-8"
) as cdc:
    for line in cdc:
        print(count)
        count += 1
                ...
                if file.upper().endswith(".WKT"):
                    ...
                    if not os.path.exists(docelowe):
                        try:
                            ...
                            with io.open(
                                r"D:\_MACIEK_\python_proby\p33\mozna_usunac.txt",  # noqa
                                "a",
                                encoding="utf-8",
                            ) as usun:
                                usun.write(plik + "\n")
                            ...
                            with io.open(
                                r"D:\_MACIEK_\python_proby\p33\bledy_kopiowania.txt",  # noqa
                                "a",
                                encoding="utf-8",
                            ) as bledy:
                                bledy.write(plik + "\n")
                        ...
                        with io.open(
                            r"D:\_MACIEK_\python_proby\p33\wkt_juz_istnieje.txt",  # noqa
                            "a",
                            encoding="utf-8",
                        ) as istnieje:
                            istnieje.write(plik + "\n")
...
with open(r"D:\_MACIEK_\python_proby\p33\kilka_dopasowan.txt", "r") as kilka:
    for line in kilka:
        ...
        if obreb.upper() in (line.split("\t")[1]).upper():
            with io.open(
                r"D:\_MACIEK_\python_proby\p33\wyczyszczone.txt",
                "a",
                encoding="utf-8",
            ) as czyste:
                czyste.write(line)
            ...
            for file in natsorted(files):
                if file.endswith(".wkt"):
                    ...
                    if (
                        ...
                        print("\t\t" + str(braki))
                        with io.open(
                            r"D:\_MACIEK_\python_proby\p33\wkt_bez_odpowiednika_pdf.txt",  # noqa
                            "a",
                            encoding="utf-8",
                        ) as brak:
                            brak.write(wkt + "\n")
        ...
        for subdir, dirs, files in os.walk(sciezka):
            ...
            if os.path.exists(wkt_operatu):
                ...
                if any(
                    ...
                    with open(
                        r"D:\_MACIEK_\python_proby\p33\brak_wkt_operatu.txt",
                        "a",
                    ) as bledy:
                        bledy.write(subdir + "\n")
            ...
            for file in natsorted(files):
                if regex.match(r".+((-M-)|(-SZK-)).+\.PDF", file.upper()):
                    ...
                    if not os.path.exists(new_wkt):
                        with open(
                            r"D:\_MACIEK_\python_proby\p33\brak_wkt_dla_plikow.txt",  # noqa
                            "a",
                        ) as bledy:
                            bledy.write(new_wkt + "\n")
            if not os.path.exists(wkt_operatu):
                with open(
                    r"D:\_MACIEK_\python_proby\p33\brak_wkt_operatu.txt", "a"
                ) as bledy:
                    bledy.write(subdir + "\n")
