    ...
    if fnmatch.fnmatch(subdir, "*;*"):
        ...
        if os.path.exists(os.path.join(subdir, "opis.txt")):
            ...
            if os.path.exists(os.path.join(oryginal, "opis.txt")):
                ...
                if file == glowny:
                    with open(
                        r"D:\python_proby\powtarzajace_identyczne_opisy.txt",
                        "a",
                    ) as po:
                        po.write(
                            "\t"
                    ...
                    with open(
                        r"D:\python_proby\powtarzajace_nie_te_same_opisy.txt",
                        "a",
                    ) as po:
                        po.write(oryginal + "\n" + subdir + "\n\n")
            for subdir, dirs, _ in os.walk(tutaj):
                    ...
                    if re.match("^. + " + num + ";.+", subdir):
                        if not subdir == d:
                            ...
                            if czy_bylo not in lista:
                                ...
                                if os.path.exists(
                                    os.path.join(subdir, "opis.txt")
                                ):
                                    lista.add(d + subdir)
                                    with open(
                                        os.path.join(subdir, "opis.txt"), "r"
                                    ) as file:
                                        file = file.read()
                                    if file == glowny:
                                        with open(
                                            r"D:\python_proby\powtarzajace_identyczne_opisy.txt",  # noqa
                                            "a",
                                        ) as po:
                                            po.write(
                                                "\t"
                                        ...
                                        with open(
                                            r"D:\python_proby\powtarzajace_nie_te_same_opisy.txt",  # noqa
                                            "a",
                                        ) as po:
                                            po.write(
                                                d + "\n" + subdir + "\n\n"
