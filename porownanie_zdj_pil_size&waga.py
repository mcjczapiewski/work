            ...
            for subdir, dirs, files in os.walk(porownanie):
                ...
                for file in sorted(files):
                    ...
                    if file.endswith(('.jpg', '.jpeg', '.JPG', '.JPEG')):
                        ...
                        # aby mogly byc te same obrazy, w naszym przypadku beda tez te same nazwy plikow,
                        # a wiec:
                        if (nrope + pierwotne) == (os.path.basename(subdir) + porownane):
                            print('NAZWY TE SAME')

                            # sprawdza czy plik oryginalny nie byl juz sprawdzany z drugim w odwrotnej
                            # kolejnosci (a,b = b,a), jak byly to bierze kolejny plik
                            czy_bylo = str(drugie + '\t' + pierwsze)
                            if czy_bylo in lines_seen:
                                continue

                            # jesli jeszcze nie byly to dodaje ta kombinacje do pamieci, zeby przy
                            # kolejnych plikach znow sprawdzic
                            else:
                                line = str(pierwsze + '\t' + drugie)
                                lines_seen.add(line)

                                # jesli sciezka pierwszego pliku zgadza sie ze sciezka drugiego to ich
                                # nie powrownuje (ten sam plik)
                                if pierwsze == drugie:
                                    continue
                                ...
                                # jesli jeszcze nie byly ze soba porownane, to je porownuje
                                    ...
                                    # jak nie otworzy zdjecia to przechodzi do kolejnego po zapisaniu
                                    # bledu do pliku tekstowego
                                    except:
                                        with open(bledny, 'a') as bl:
                                ...
                                # podejmuje probe porownania
                                    try:
                                        ...
                                        if (len(original.fp.read()) == len(image_to_compare.fp.read())
                                                and original.size == image_to_compare.size):
                                            ...
                                            with open(bledny, 'a') as bl:
                                                bl.write(pierwsze + '\t' + drugie + '\trozne\n')
                                            continue
