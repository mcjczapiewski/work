# -*- coding: utf-8 -*-
import cv2, os, datetime, fitz, regex, io
...
plikwynik = sciezka+'\\'+wynikowy+'_strony_'+czasstart.strftime('%Y-%m-%d')+'.txt'
...
for subdir, dirs, files in os.walk(tutaj):
    ...
    if not any(fname.upper().endswith('.PDF') for fname in os.listdir(subdir)):
        continue
    nrope = os.path.basename(os.path.dirname(subdir))+'_'+os.path.basename(subdir)
    print(str(countope)+'\t'+nrope)
##    print(str(ile)+'_'+str(countope)+'\t'+nrope)
    countope += 1
    for file in natsorted(files):
        if file.upper().endswith('.PDF'):
##        if regex.match('^.+(-SZK-|-M-).+\.PDF', file.upper()):
...
##                if not strony == 1:
##                with io.open(plikwynik, 'a', encoding = 'utf-8') as wynik:
