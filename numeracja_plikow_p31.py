...
for subdir, dirs, files in os.walk(sciezka):
        ...
        if file.upper().endswith('.PDF'):
            ...
            if not numer == kolejny:
                ...
                try:
                    ...
                    try:
                        aa_nazwa = os.path.join(subdir, operat+str(kolejny)+'aaa'+dokument)
                        os.rename(plik, aa_nazwa)
                        aa.add(aa_nazwa)
                    except:
                        try:
                            bb_nazwa = os.path.join(subdir, operat+str(kolejny)+'bbb'+dokument)
                            os.rename(plik, bb_nazwa)
                            bb.add(bb_nazwa)
                        except:
                            with open(os.path.join(sciezka, 'bledy.txt'), 'a') as bl:
                                bl.write(plik+'\t'+nazwa+'\tNie udało się zmienić nazwy pliku.\n')
                            
                wkt = os.path.join(subdir, os.path.splitext(file)[0]+'.wkt')
                if os.path.exists(wkt) and any(i in wkt for i in (('SZK-POL', 'M-WYN', 'M-WYW', 'M-UZ'))):
                    ...
                    try:
                        ...
                        try:
                            aa_nazwa = os.path.join(subdir, operat+str(kolejny)+'aaa'+os.path.splitext(dokument)[0]+'.wkt')
                            os.rename(plik, aa_nazwa)
                            aa.add(aa_nazwa)
                        except:
                            try:
                                bb_nazwa = os.path.join(subdir, operat+str(kolejny)+'bbb'+os.path.splitext(dokument)[0]+'.wkt')
                                os.rename(plik, bb_nazwa)
                                bb.add(bb_nazwa)
                            except:
                                with open(os.path.join(sciezka, 'bledy.txt'), 'a') as bl:
                                    bl.write(wkt+'\t'+os.path.splitext(nazwa)[0]+'.wkt'+'\tNie udało się zmienić nazwy pliku.\n')
            kolejny += 1
    for i in aa:
        try:
            ...
            with open(os.path.join(sciezka, 'bledy.txt'), 'a') as bl:
                bl.write(i+'\t'+i.split('aaa')[0]+i.split('aaa')[1]+'\tNie udało się zmienić nazwy pliku.\n')
    ...
    for i in bb:
        try:
            ...
            with open(os.path.join(sciezka, 'bledy.txt'), 'a') as bl:
                bl.write(i+'\t'+i.split('bbb')[0]+i.split('bbb')[1]+'\tNie udało się zmienić nazwy pliku.\n')
            ...
            if file.upper().endswith('.PDF'):
                ...
                if typ not in duble:
                    ...
                    if not str(pierwotna) == str(do_tego).zfill(3):
                        ...
                        try:
                            os.rename(os.path.join(subdir, file), os.path.join(subdir, bedzie))
                        except:
                            try:
                                aa_nazwa = os.path.join(subdir, bylo+'aaa'+str(do_tego).zfill(3)+rozszerzenie)
                                os.rename(plik, aa_nazwa)
                                aa.add(aa_nazwa)
                            except:
                                try:
                                    bb_nazwa = os.path.join(subdir, bylo+'bbb'+str(do_tego).zfill(3)+rozszerzenie)
                                    os.rename(plik, bb_nazwa)
                                    bb.add(bb_nazwa)
                                except:
                                    with open(os.path.join(sciezka, 'bledy.txt'), 'a') as bl:
                                        bl.write(os.path.join(subdir, file)+'\t'+os.path.join(subdir, bedzie)+'\tNie udało się zmienić nazwy pliku.\n')
                        wkt = os.path.join(subdir, os.path.splitext(file)[0]+'.wkt')
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        with open(os.path.join(sciezka, 'bledy.txt'), 'a') as bl:
                                            bl.write(wkt+'\t'+os.path.join(subdir, bedzie)+'\tNie udało się zmienić nazwy pliku.\n')
                    duble[typ] = do_tego
    ...
    for _, _, files in os.walk(subdir):
        for file in natsorted(files):
            if file.upper().endswith(('.TXT', '.KCD', '.DXF', '.DWG')):
                nazwa = file.split(os.path.basename(subdir))[1]
                nr_tomu = 0
                if regex.match('^.T.+', nazwa):
                    nr_tomu = 1
                    tom = regex.match('^.(T.*?)(_[1-9]|-[1-9])', nazwa)[1]
                    nazwa = regex.match('^.T.*?((_[1-9].+$)|-[1-9].+$)', nazwa)[1]
                if nazwa.startswith('-'):
                ...
                if rozszerz not in ponownie:
                    ...
                    do_tego += 1
                    duze_litery = file.upper()
                    pierwotna = regex.match('^.+-(.+)'+rozszerz.upper(), duze_litery)[1].zfill(3)
                    if not str(pierwotna) == str(do_tego).zfill(3):
                        print(str(count)+'\t'+file)
                        count += 1
                        bylo = file.split(pierwotna)[0]
