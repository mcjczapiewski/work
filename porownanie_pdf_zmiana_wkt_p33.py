# -*- coding: utf-8 -*-
import cv2, os, datetime, fitz, io
                ...
                for file in natsorted(files):
                    if file.endswith('.pdf') or file.endswith('.PDF'):
                        ...
                        for subdir, dirs, files in os.walk(idz_tu):
                            dirs.sort(key=nkey)
                            nazwafolder = os.path.basename(subdir)
                            folder_drugi = subdir
                                ...
                                for file in natsorted(files):
                                    if file.endswith('.pdf') or file.endswith('.PDF'):
                                        ...
                                        try:
                                            ...
                                            with io.open(bledny, 'a', encoding = 'utf-8') as bl:
                                                bl.write('Nie udało się otworzyć PORÓWNYWANEGO:\t'+drugie+'\n')
                                            continue
        ##                                try:
                    ...
                    for file in natsorted(files):
                        if file == starawkt:
                            ...
                            try:
                                ...
                                subnewname = os.path.splitext(nowa)[0]+'a.wkt'
                                try:
        ...
        for line in natsorted(usun_a):
            ...
            try:
                ...
                with io.open(blednywkt, 'a', encoding = 'utf-8') as bl:
                    bl.write('Nie mogłem zmienić nazwy, plik o takiej samej nazwie chyba wciąż istnieje:\t'+line)
    czaskoniec = datetime.datetime.now()
    roznicaczas = czaskoniec - czasstart
    czastrwania = roznicaczas.total_seconds()/60
...
corobimy = 0
while corobimy == 0:
    print('\nCzy chcesz:\n1.\tPorównywać PDFy?\n2.\tPDFy są już porównane i jest plik tekstowy ze zmianami w nazwach, więc idziemy od razu do WKT.')
    tak_wiec = input('\nWpisz 1 lub 2: ')
    if tak_wiec == '1':
        corobimy = 1
        porownajpdf()
