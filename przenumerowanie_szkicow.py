...
zmienione = sciezka+'\\'+os.path.basename(os.path.normpath(sciezka))+'_zmienione_'+czasstart.strftime('%Y-%m-%d')+'.txt'
            ...
            if fnmatch.fnmatch(file, '00_*'):
                ...
                if newname == file:
                    ...
                    try:
                        ...
                        #jesli podczas proby okazalo sie, ze taka nazwa juz istnieje, to tworzy kolejna nazwe dodajac do numeru strony 'a'
                    except FileExistsError:
                        subnewname = a[0]+'_'+str(nrstr).zfill(3)+'a_'+a[2]

                            #przenazwnie z nowa nazwa
                        os.rename(os.path.join(subdir, file), os.path.join(subdir, subnewname))
                        with open(zmienione, 'a') as zm:
                            if sciezkapisz == 1:
                                zm.write('\n\n\n\n'+subdir+'\n\n')
                        ...
                        sciezkapisz = 0
                            ...
                            #dodanie sciezki do przenazwanego pliku z 'a' zeby na koniec to 'a' usunac
                        usun_a.add(os.path.join(subdir, subnewname))

                        #jesli nie przenazwal nawet po dodaniu 'a' to wpisuje blad do pliku
                    except:
                        with open(bledny, 'a') as bl:
                            bl.write('Jakiś inny błąd w:\t'+os.path.join(subdir, file)+'\n')

                    nrstr += 1

            else:
                with open(bledny, 'a') as bl:
                    bl.write('Nie zmieniono nazwy, bo nie odpowiada wzorcowi:\t'+os.path.join(subdir, file)+'\n')


        #jesli lista od usun_a nie jest pusta to usuwa a z plikow tam wystepujacych    
    if usun_a:
        with open(zmienione, 'a') as zm:
            zm.write('\n\n\n\n')
        ...
        for line in natsorted(usun_a):
            ...
            try:
                ...
                os.rename(line, os.path.join(os.path.dirname(line), subnewname))
                with open(zmienione, 'a') as zm:
                            zm.write(os.path.basename(line)+'\t\t'+subnewname+'\n')
