    ...
    if czyjpg == 1:
        ...
        try:
            with open(spispdf, 'wb') as sp:
                sp.write(img2pdf.convert([os.path.join(subdir, file) for file in natsorted(files) if fn.fnmatch(file, '*SPIS*')]))
        except:
            with open(bledy, 'a') as bl:
                bl.write(os.path.join(subdir, file)+'\tnie dodany do PDFa\n')
        try:
            with open(zmpdf, 'wb') as zp:
                zp.write(img2pdf.convert([os.path.join(subdir, file) for file in natsorted(files) if not fn.fnmatch(file, '*SPIS*')]))
        except:
            with open(bledy, 'a') as bl:
                bl.write(os.path.join(subdir, file)+'\tnie dodany do PDFa\n')
