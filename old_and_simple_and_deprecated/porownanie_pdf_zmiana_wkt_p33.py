...
def porownajpdf():
    ...
    with open(r"D:\_MACIEK_\python_proby\p33\sciezki.txt", "r") as sciezki:
        for line in sciezki:
            ...
            for subdir, dirs, files in os.walk(sprawdzanie):
                dirs.sort(key=nkey)
                if not any(
                    fname.endswith(".PDF") for fname in os.listdir(subdir)
                ):
                    continue
                folder_glowny = subdir
                dopasuj = os.path.basename(
                    os.path.dirname(folder_glowny)
                ) + os.path.basename(folder_glowny)
                czy_znalazl = 0
                szybciej = "a"
                byly_juz = set()
                    ...
                    if file.endswith(".pdf") or file.endswith(".PDF"):
                        ...
                        try:
                            ...
                            with io.open(bledny, "a", encoding="utf-8") as bl:
                                bl.write(
                                    "Nie udało się otworzyć PIERWOTNEGO:\t"
                                    + pierwsze
                                    + "\n"
                                )
                            continue
                ...
                if czy_znalazl == 0:
                    with io.open(bledny, "a", encoding="utf-8") as bl:
                        bl.write(
                            "NIE ZNALEZIONO FOLDERU O TAKIEJ SAMEJ NAZWIE!:\t"
                            + folder_glowny
                            + "\n"
                        )
    ...
    print(
        "\nSkończyłem porównywać PDFy. Mam listę zmian w pliku.\
            Ta część zajęła (minuty):"
    )
                    ...
                    for file in natsorted(files):
                        if file == starawkt:
                            ...
                            try:
                                ...
                                subnewname = (
                                    os.path.splitext(nowawkt)[0] + "a.wkt"
                                )
                                try:
                                    os.rename(
                                        do_zmiany,
        ...
        for line in natsorted(usun_a):
            ...
            try:
                ...
                with io.open(blednywkt, "a", encoding="utf-8") as bl:
                    bl.write(
                        "Nie mogłem zmienić nazwy, plik o takiej \
                            samej nazwie chyba wciąż istnieje:\t"
                        + line
                    )
    czaskoniec = datetime.datetime.now()
    roznicaczas = czaskoniec - czasstart
