                ...
                if file.upper().endswith('.WKT'):
                    ...
                    if not os.path.exists(docelowe):
                        try:
                            ...
                            with io.open(r'D:\_MACIEK_\python_proby\p33\mozna_usunac.txt',
                                         'a', encoding='utf-8') as usun:
                                usun.write(plik + '\n')
                        except:
                            with io.open(r'D:\_MACIEK_\python_proby\p33\bledy_kopiowania.txt',
                                         'a', encoding='utf-8') as bledy:
                                bledy.write(plik + '\n')
                    else:
                        with io.open(r'D:\_MACIEK_\python_proby\p33\wkt_juz_istnieje.txt',
                                     'a', encoding='utf-8') as istnieje:
                            istnieje.write(plik + '\n')
...
with open(r'D:\_MACIEK_\python_proby\p33\kilka_dopasowan.txt', 'r') as kilka:
    for line in kilka:
        obreb = (regex.sub(r'^.+do_operatow.+\.[0-9][0-9][0-9][0-9](.+?)\t.+', '\\1', line)).split('\n')[0]
        ...
        for subdir, dirs, files in os.walk(sciezka):
            ...
            for file in natsorted(files):
                if file.endswith('.wkt'):
                    ...
                    print(str(count) + '\t' + wkt)
                    count += 1
                    if (not os.path.exists((wkt.split('.wkt')[0]) + '.PDF')
                            and os.path.basename(subdir) != file.split('.wkt')[0]):
                        braki += 1
                        print('\t\t' + str(braki))
                        with io.open(r'D:\_MACIEK_\python_proby\p33\wkt_bez_odpowiednika_pdf.txt',
                ...
                for line in sprawdz:
                    if zeruj == 0:
                        ...
                        if not regex.match(r'^POLYGON|MULTI.*', line.upper()):
                            braki += 1
                            print('\t\t' + str(braki))
                            with open(r'D:\_MACIEK_\python_proby\p33\wkt_bledne_struktuaaary.txt',
                                      'a') as brak:
                                brak.write(wkt + '\n')
                    else:
                        continue
    ...
    for file in natsorted(files):
        if file.endswith('.xml'):
            ...
            if 'utf-8' not in str(detector.result):
                with open(r'D:\_MACIEK_\python_proby\p33\numer_p_do_xml\kodowania_plikow.txt',
                          'a') as kodowanie:
    ...
    for line in sciezki:
        ...
        for subdir, dirs, files in os.walk(sciezka):
            ...
            if os.path.exists(wkt_operatu):
                for file in natsorted(files):
                    if regex.match(r'.+((-M-)|(-SZK-)).+\.PDF', file.upper()):
