...
for subdir, dirs, files in os.walk(sciezka):
    ...
    for file in natsorted(files):
        if file.upper().endswith('.PDF'):
            ...
            if regex.match(r'^.T.+', bez_ope):
                ...
                bez_ope = regex.match(r'^.T.*?((_[1-9].+$)|-[1-9].+$)', bez_ope)[1]
            ...
            if not numer == kolejny:
                ...
                if os.path.exists(wkt) and any(i in wkt for i in (('SZK-POL', 'M-WYN', 'M-WYW', 'M-UZ'))):
                    ...
                    try:
                        ...
                        try:
                            aa_nazwa = os.path.join(subdir, operat + str(kolejny)
                                                    + 'aaa' + os.path.splitext(dokument)[0] + '.wkt')
                            os.rename(plik, aa_nazwa)
                            aa.add(aa_nazwa)
                        except:
                            try:
                                bb_nazwa = os.path.join(subdir, operat + str(kolejny)
                                                        + 'bbb' + os.path.splitext(dokument)[0] + '.wkt')
                                os.rename(plik, bb_nazwa)
                                bb.add(bb_nazwa)
                            except:
                                with open(os.path.join(sciezka, 'bledy.txt'), 'a') as bl:
                                    bl.write(wkt + '\t' + os.path.splitext(nazwa)[0] + '.wkt')
                                    bl.write('\tNie udało się zmienić nazwy pliku.\n')
            kolejny += 1
    for i in aa:
        try:
            ...
            with open(os.path.join(sciezka, 'bledy.txt'), 'a') as bl:
                bl.write(i + '\t' + i.split('aaa')[0] + i.split('aaa')[1])
                bl.write('\tNie udało się zmienić nazwy pliku.\n')
    ...
    for i in bb:
        try:
            ...
            with open(os.path.join(sciezka, 'bledy.txt'), 'a') as bl:
                bl.write(i + '\t' + i.split('bbb')[0] + i.split('bbb')[1])
                bl.write('\tNie udało się zmienić nazwy pliku.\n')
    ...
    for _, _, files in os.walk(subdir):
        for file in natsorted(files):
            if file.upper().endswith('.PDF'):
                ...
                typ = regex.match(r'.+?([A-Z].*[A-Z])', str(os.path.splitext(nazwa)[0]))[1]
                if typ not in duble:
                    duble[typ] = 1
                elif typ in duble:
                    do_tego = duble[typ]
                    ...
                    pierwotna = regex.match(r'^.+-(.+)\.PDF', duze_litery)[1].zfill(3)
                    if not str(pierwotna) == str(do_tego).zfill(3):
                        ...
                        try:
                            ...
                            try:
                                aa_nazwa = os.path.join(subdir, bylo + 'aaa' + str(do_tego).zfill(3)
                                                        + rozszerzenie)
                                os.rename(plik, aa_nazwa)
                                aa.add(aa_nazwa)
                            except:
                                try:
                                    bb_nazwa = os.path.join(subdir, bylo + 'bbb' + str(do_tego).zfill(3)
                                                            + rozszerzenie)
                                    os.rename(plik, bb_nazwa)
                                    bb.add(bb_nazwa)
                                except:
                                    with open(os.path.join(sciezka, 'bledy.txt'), 'a') as bl:
                                        ...
                                        bl.write(os.path.join(subdir, bedzie))
                                        bl.write('\tNie udało się zmienić nazwy pliku.\n')
                        wkt = os.path.join(subdir, os.path.splitext(file)[0] + '.wkt')
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        with open(os.path.join(sciezka, 'bledy.txt'), 'a') as bl:
                                            bl.write(wkt + '\t' + os.path.join(subdir, bedzie))
                                            bl.write('\tNie udało się zmienić nazwy pliku.\n')
                    duble[typ] = do_tego
        ...
        for file in natsorted(files):
            if file.upper().endswith(('.TXT', '.KCD', '.DXF', '.DWG')):
                ...
                if regex.match(r'^.T.+', nazwa):
                    ...
                    nazwa = regex.match(r'^.T.*?((_[1-9].+$)|-[1-9].+$)', nazwa)[1]
                if nazwa.startswith('-'):
                ...
                if rozszerz not in ponownie:
                    ...
                    do_tego += 1
                    duze_litery = file.upper()
                    pierwotna = regex.match(r'^.+-(.+)' + rozszerz.upper(), duze_litery)[1].zfill(3)
                    if not str(pierwotna) == str(do_tego).zfill(3):
                        print(str(count) + '\t' + file)
                        count += 1
                        bylo = file.split(pierwotna)[0]
