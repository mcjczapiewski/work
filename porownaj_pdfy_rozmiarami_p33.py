            ...
            if (file.upper()).endswith('.PDF'):
                ...
                if os.path.exists(o_pdf):
                    ...
                    if not o_rozmiar-5000 <= n_rozmiar <= o_rozmiar+5000:
                        with io.open(os.path.join(sciezka, 'rozne_wagi_plikow.txt'), 'a', encoding = 'utf-8') as wagi:
                            wagi.write(n_pdf+'\t'+o_pdf+'\n')
                        continue
                    else:
                        with io.open(os.path.join(sciezka, 'wagi_plikow_sa_takie_same.txt'), 'a', encoding = 'utf-8') as same:
                            same.write(n_pdf+'\t'+o_pdf+'\n')
                else:
                    with io.open(os.path.join(sciezka, 'takie_pdf_nie_istnialy.txt'), 'a', encoding = 'utf-8') as new:
                        new.write(n_pdf+'\n')
    for _, _, files in os.walk(o_folder):
        for file in natsorted(files):
            if (file.upper()).endswith('.PDF'):
                if not file in zbior_pdf:
                    o_pdf = os.path.join(o_folder, file)
                    with io.open(os.path.join(sciezka, 'kiedys_byly_teraz_nie_ma.txt'), 'a', encoding = 'utf-8') as brak:
                        brak.write(o_pdf+'\n')
...
for subdir, dirs, _ in os.walk(oryginal):
    ...
    if not subdir in juz_byly:
        ...
        count += 1
        with io.open(os.path.join(sciezka, 'foldery_ktorych_nie_ma_w_nowych.txt'), 'a', encoding = 'utf-8') as braki:
            braki.write(subdir+'\n')
...
##                    print(count)
##                    count += 1
##                    with io.open(os.path.join(sciezka, 'ale_sa_z_innym_nr_dok.txt'), 'a', encoding = 'utf-8') as inne:
