...
for subdir, dirs, _ in os.walk(nowe):
    ...
    if not any(fname.endswith(".PDF") for fname in os.listdir(subdir)):
        continue
    n_folder = subdir
    dopasuj = os.path.basename(os.path.dirname(n_folder)) + os.path.basename(
        n_folder
    )
    zbior_pdf = set()
    ...
    if znaleziono == 0:
        with io.open(
            os.path.join(sciezka, "nie_bylo_takiego_folderu.txt"),
            "a",
            encoding="utf-8",
        ) as heh:
            heh.write(n_folder + "\n")
            ...
            if (file.upper()).endswith(".PDF"):
                ...
                if os.path.exists(o_pdf):
                    ...
                    if not o_rozmiar - 5000 <= n_rozmiar <= o_rozmiar + 5000:
                        with io.open(
                            os.path.join(sciezka, "rozne_wagi_plikow.txt"),
                            "a",
                            encoding="utf-8",
                        ) as wagi:
                        ...
                        with io.open(
                            os.path.join(
                                sciezka, "wagi_plikow_sa_takie_same.txt"
                            ),
                            "a",
                            encoding="utf-8",
                        ) as same:
                    ...
                    with io.open(
                        os.path.join(sciezka, "takie_pdf_nie_istnialy.txt"),
                        "a",
                        encoding="utf-8",
                    ) as new:
                        new.write(n_pdf + "\n")
        ...
        for file in natsorted(files):
            if (file.upper()).endswith(".PDF"):
                if file not in zbior_pdf:
                    ...
                    with io.open(
                        os.path.join(sciezka, "kiedys_byly_teraz_nie_ma.txt"),
                        "a",
                        encoding="utf-8",
                    ) as brak:
                        brak.write(o_pdf + "\n")
...
for subdir, dirs, _ in os.walk(oryginal):
    ...
    if subdir not in juz_byly:
        ...
        count += 1
        with io.open(
            os.path.join(sciezka, "foldery_ktorych_nie_ma_w_nowych.txt"),
            "a",
            encoding="utf-8",
        ) as braki:
            braki.write(subdir + "\n")
...
with io.open(
    ...
    for line in sprawdz:
        ...
        if os.path.exists(wkt):
            ...
            count += 1
            with io.open(
                os.path.join(sciezka, "teraz_nie_ma_mial_wkt.txt"),
                "a",
                encoding="utf-8",
            ) as oj:
                oj.write(pdf + "\t" + wkt + "\n")
...
with io.open(
    ...
    for line in sprawdz:
        line = (line.split("\n")[0]).replace(
            r"I:\INOWROCŁAW\DANE_IRON_MOUNTAIN\20190614", "H:"
        )
        ...
        for subdir, dirs, files in os.walk(tutaj):
            for file in natsorted(files):
                if regex.match("^.*" + moze_jednak, file):
                    ...
                    count += 1
                    with io.open(
                        os.path.join(sciezka, "ale_sa_z_innym_nr_dok.txt"),
                        "a",
                        encoding="utf-8",
                    ) as inne:
