...
def sciezki():
    ...
    resztawkt = sciezka+'\\'+os.path.basename(os.path.normpath(sciezka))+'_nieskopiowane_'+czasstart.strftime('%Y-%m-%d')+'.txt'
    ...
    for subdir, dirs, _ in os.walk(tusapdfy):
        ...
        for subdir, dirs, files in os.walk(tusawkt):
            ...
            for file in natsorted(files):
                if file.endswith('.wkt') and re.match('^'+nrope+'(_|-| |\.wkt)', file):
                    
                    wktpath = os.path.join(folderpdf, file)
                    plikwkt = os.path.join(subdir, file)

                        #sprawdzenie czy przypadkiem taki wkt juz nie istnieje, aby go nie nadpisać
                    if os.path.exists(wktpath):
                        with open(bledny, 'a') as bl:
                            bl.write('Taki plik już istnieje:\t'+plikwkt+'\n')
                    else:
                            #jak nie istnieje to kopiuje wkt do lokalizacji z operatem
                        try:
                            shutil.copy(plikwkt, folderpdf)
                            listawkt.add(plikwkt)
                        except:
                            with open(bledny, 'a') as bl:
                                bl.write('Nie udało się skopiować pliku:\t'+plikwkt+'\n')

        #sprawdza, czy wszystkie pliki WKT znalazły swoje odpowiedniki w folderach z operatami
    for subdir, dirs, files in os.walk(tusawkt):
            dirs.sort(key=nkey)
            for file in natsorted(files):
...
def operaty():
    ...
    for subdir, dirs, _ in os.walk(tusapdfy):
        ...
        for subdir, dirs, files in os.walk(tusawkt):
            ...
            #jeśli nie znajdzie w folderze z operatem żadnego wkt, tzn. że wkt dla danego operatu nie istnieje
        ...
        if not os.path.exists(plikwkt):
            with open(bledny, 'a') as bl:
                bl.write('WKT dla operatu nie istnieje więc nie skopiowano go na m-wyn, m-wyw itd.:\t'+folderpdf+'\n')

            #jeśli tak, to kopiuje go dla map i szkiców
        else:
            for _, _, files in os.walk(folderpdf):
                for file in natsorted(files):
                    if re.match('(.*M-WYW.*)|(.*M-WYN.*)|(.*SZK-POL.*)', file) and file.endswith(('.PDF', '.pdf')):
                        newwkt = os.path.join(folderpdf, os.path.splitext(file)[0]+'.wkt')

                            #upewnia się, że wkt dla danej mapy/szkicu jeszcze nie istnieje, jesli nie to tworzy z głównego pliku
                ...
                #jeżeli nie ma w danym folderze wkt dla mapy wywiadu, wynikowej bądź żadnego szkicu (bo nie ma dla nich pdf)
                #to tworzy wkt dla mapy uzupełniającej (jeśli istnieje)
            if not any(re.match('(.*M-WYW.*wkt)|(.*M-WYN.*wkt)|(.*SZK-POL.*wkt)', fname) for fname in os.listdir(folderpdf)):
                for _, _, files in os.walk(folderpdf):
                    for file in natsorted(files):
                        if re.match('.*M-UZ.*', file) and file.endswith(('.PDF', '.pdf')):
                            newwkt = os.path.join(folderpdf, os.path.splitext(file)[0]+'.wkt')
                            if not os.path.exists(newwkt):
                                try:
                                    shutil.copy(plikwkt, newwkt)
        ...
        #sprawdza, które pliki wkt nie znalazły odpowienika w folderach z operatami
    for subdir, dirs, files in os.walk(tusawkt):
            dirs.sort(key=nkey)
            for file in natsorted(files):
                if file.endswith('.wkt'):
