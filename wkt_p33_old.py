...
# czy sa wkt dla plikow i operatow, jak brak glownej czy jest jakakolwiek
# czy do pliku wkt jest odpowiednik w pdf
# with open(
#     r"I:\INOWROCŁAW\DANE_IRON_MOUNTAIN\20190614\ZADANIE 5\INOWROCŁAW CAŁOŚĆ\kontrole\sciezki.txt",
#     "r",
#     encoding="utf-8",
# ) as sciezki:
#     for line in sciezki:
#         sciezka = line.strip()
sciezka = (
    r"I:\INOWROCŁAW\DANE_IRON_MOUNTAIN\20190614\ZADANIE 5\INOWROCŁAW CAŁOŚĆ"
)
for subdir, dirs, files in os.walk(sciezka):
    dirs.sort(key=nkey)
    if not any(
        fname.upper().endswith((".WKT", ".PDF"))
        for fname in os.listdir(subdir)
    ):
        continue
    ...
    if os.path.exists(wkt_operatu):
        ...
        with open(
            os.path.join(sciezka, r"kontrole\brak_wkt_dla_operatu.txt"),
            "a",
            encoding="utf-8",
        ) as bledy:
            bledy.write(subdir + "\n")
        if any(fname.upper().endswith(".WKT") for fname in os.listdir(subdir)):
            with open(
                os.path.join(
                    sciezka, r"kontrole\brak_glownej_jest_do_pliku.txt"
                ),
                "a",
                encoding="utf-8",
            ) as bledy:
    ...
    for file in natsorted(files):
        ...
        if file.endswith(".wkt") and (
            ...
            with open(
                os.path.join(
                    sciezka, r"kontrole\wkt_do_niewlasciwych_plikow.txt"
                ),
                "a",
                encoding="utf-8",
            ) as bledy:
        ...
        if file.endswith(".wkt"):
            ...
            if (
                ...
                with open(
                    os.path.join(
                        sciezka, r"kontrole\wkt_do_nieistniejacych_plikow.txt"
                    ),
                    "a",
                    encoding="utf-8",
                ) as brak:
