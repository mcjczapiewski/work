...
# sciezka = r"I:\INOWROCŁAW\DANE_IRON_MOUNTAIN\20190614\ZADANIE 4\M.JANIKOWO"
# write_out = fr"{sciezka}\kontrole\kontrola numeracji"  # noqa
    ...
    for line in sciezki:
        ...
        for subdir, dirs, files in os.walk(sciezka):
            ...
            if (
                ...
                continue
            print(str(count) + "\t" + subdir.split("04_KOPIA_PLIKOWA")[1])
            count += 1
            kolejny = 1
            for file in natsorted(files):
                if file.upper().endswith(".PDF") and file.upper().startswith("P."):
                    ...
                    try:
                        ...
                        with io.open(
                            os.path.join(write_out, "zaneta_1_nieprawidlowy_nr_operatu.txt"),
                            "a",
                            encoding="utf-8",
                        ) as bl_nazwy:
                            bl_nazwy.write(plik + "\n")
                    ...
                    try:
                        ...
                        with io.open(
                            os.path.join(write_out, "zaneta_1_bledy_w_nazwach_plikow.txt"),
                            "a",
                            encoding="utf-8",
                        ) as bl_nazwy:
                            bl_nazwy.write(plik + "\n")
                    ...
                    if not numer == kolejny:
                        ...
                        with io.open(
                            os.path.join(write_out, "zaneta_1_bledny_nr_dokumentu.txt"),
                            "a",
                            encoding="utf-8",
                        ) as dokumentu:
                    ...
                    if file.upper().endswith(".PDF") and file.upper().startswith("P."):
                        ...
                        if typ not in duble:
                            ...
                            if not str(pierwotna) == str(do_tego).zfill(3):
                                ...
                                with io.open(
                                    os.path.join(write_out, "zaneta_1_bledny_numer_strony.txt"),
                                    "a",
                                    encoding="utf-8",
                                ) as strony:
