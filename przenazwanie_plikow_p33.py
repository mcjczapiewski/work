        ...
        with io.open(przenazwanie, 'r', encoding='utf-8') as zmiany:
            for line in zmiany:
                ...
                if dwa in subdir and file == bylo:
                    ...
                    nowe = os.path.join(subdir, dwa + '_' + str(dok) + '-' + bedzie + '-'
                                        + str(strona).zfill(3) + os.path.splitext(file)[1])

                    if os.path.exists(os.path.join(subdir, os.path.splitext(file)[0] + '.wkt')):
                        wkt = os.path.join(subdir, os.path.splitext(file)[0] + '.wkt')
                        wkt_nowe = os.path.join(subdir, dwa + '_' + str(dok) + '-' + bedzie + '-'
                                                + str(strona).zfill(3) + os.path.splitext(wkt)[1])
                        try:
